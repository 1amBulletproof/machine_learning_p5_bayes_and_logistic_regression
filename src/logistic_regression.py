...
class LogisticRegression(BaseModel):
	...
	def train(self, train_data, learning_rate=0.5):
		...
		while(weights_changed):
			...
			#TODO: exit after a number of iterations
				
		return self.class_weights
	
	'''
		#Need to create a weight vector for each class
		for class_idx in range(num_classes):
			new_weight_vector = self.initialize_weights(self.data.columns, 0.01)
			new_delta_weight_vector = pd.Series([0] * self.data.columns.length)

			#Run this algorithm until the weights are unchanged
			weights_unchanged = False

			while(weights_unchanged):
				#For each input point, update the weight delta vector
				for point in self.data:
					summation = self.get_summation_point_and_weight(point, new_weight_vector)
					estimated_result = self.get_sigmoid(summation)
					new_delta_weight_vector = self.get_weight_delta_vector(point, estimated_result, new_delta_weight_vector)
	
				#if weights unchanged, add this final weight vector to the weight_vectors list
				if (new_delta_weight_vector.unique().length == 1 and new_delta_weight_vector[0] = 0):
					weight_vectors.append(new_weight_vector.tolist())
					weights_unchanged = True

				#if weights changed, update the weight vector with the weight delta vector and repeat
				else:
					new_weight_vector = self.update_weight_vector(new_weight_vector, new_delta_weight_vector, learning_factor)
					'''
	...
	#@return	result
	#=============================
	def get_sigmoid(self, input_val):
		return 1 / (1 + np.exp(-input_val))

	#=============================
	# get_summation_point_and_weight()
	#	- essentially SUM(Xi*Wi)
	#
	#@param	input	input vector
	#@param	weights	input vector
	#@return	result
	#=============================
	def get_summation_weight_and_values(self, input, weights):
		result = 0
		return result

	#=============================
	# get_weight_delta_vector()
	#	- calculate the weight delta and return it
	#
	#@param	input_vector
	#@param	estimated_result
	#@param	weight_delta_vector
	#@return	updated weight delta vector
	#=============================
	def get_weight_delta_vector(self, input_vector, estimated_result, weight_delta_vector ):
		return weight_delta_vector

	#=============================
	# update_weight_vector()
	#	- update the weight vector wiht the new weights
	#
	#@param	weight_vector
	#@param	weight_delta_vector
	#@param	learning_rate
	...
	#=============================
	def update_weight_vector(self, weight_vector, weight_delta_vector, learning_rate ):
		return updated_weight_vector

	#=============================
	# initialize_weights()
	#	- initialize a vector of weights
	#
	#@param	vector_length
	#@param	range_of_values (+/- this val)
	#@return	initial weight vector
	#=============================
	def initialize_weights(self, vector_length, range_of_values ):
